...
export default function SearchBox() {
  ...
  return (
    <div className="relative w-full flex">
      ...
      {/* Suggestions Overlay */}
      {showSuggestions && (
  <div className={`fixed top-24 rounded-t-lg  inset-0 bg-white z-30 overflow-auto ${showSuggestions ? 'animate-slide-up' : 'animate-slide-down'}`}>
          {/* Mini Navbar */}
          <div className='flex py-2 px-4 justify-between'>
            <h1 className="text-lg font-bold text-red-600">Dhanya Builders</h1>
            <button
          ...
          {recentSearches.length === 0 && suggestions.length === 0 && <p className='text-xs text-center mt-40 text-gray-400 italic mb-10'>No Recent Searches..</p>}
            ...
            <ul className={`grid grid-cols-2 ${currentUrl.toLowerCase().includes('/search/name/') ? 'md:grid-cols-1 lg:grid-cols-1' : 'md:grid-cols-1 lg:grid-cols-1'} gap-4`}>
